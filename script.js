...
// Form submission handled via setupModalForm helper below
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!joinUsBtn || joinUsBtn.closest("form")) {
    return
  }
...
function setupModalForm({
  ...
  const dismissBtn = modal.querySelector(dismissSelector)
  const closeBtn = modal.querySelector(closeSelector)
  const confettiCanvas = confettiSelector
    ? modal.querySelector(confettiSelector) || document.querySelector(confettiSelector)
  ...
  const startConfetti = () => {
    ...
    const colors = ['#96BC21', '#ff6b6b', '#ffd93d', '#2ec4b6', '#5a7bff']
    const pieces = Array.from({ length: 180 }, () => ({
      ...
      rotationStep: -0.06 + Math.random() * 0.12,
    ...
    const draw = () => {
      ...
      pieces.forEach((piece) => {
        ...
        piece.y += piece.vy
        piece.vy += 0.12
        piece.rotation += piece.rotationStep

        if (piece.x < -50) piece.x = width + 30
        if (piece.x > width + 50) piece.x = -30
        if (piece.y > height + 80) {
          piece.y = -20
          piece.vy = 3 + Math.random() * 3
        }
    ...
    draw()
    confettiTimeout = window.setTimeout(stopConfetti, 3200)
  }
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const originalMarkup = submitButton ? submitButton.innerHTML : null
    if (submitButton) {
    ...
    if (emailValue && !formData.has('_replyto')) {
    ...
    if (!formData.has('name')) {
      ...
      if (combined) {
        formData.append('name', combined)
      }
    }

    const action = form.getAttribute('action') || ''
    const requestUrl = action.includes('/ajax/')
      ? action
    ...
    try {
      ...
      const payloadIndicatesSuccess = () => {
        if (!payload) return true
        if (payload.success === true || payload.success === 'true') return true
        if (typeof payload.success === 'undefined') return true
        return false
      }

      const success = response.ok && payloadIndicatesSuccess()
      ...
      if (success) {
        ...
        if (payload) {
          if (Array.isArray(payload.errors) && payload.errors.length > 0) {
            message = payload.errors.map((err) => err.message || err).join(', ')
        ...
        console.error('FormSubmit error', { status: response.status, payload })
      }
    } catch (error) {
      setStatus('Network error. Please try again.', 'error')
      console.error('FormSubmit network error', error)
    } finally {
      if (submitButton && originalMarkup !== null) {
...
document.addEventListener('DOMContentLoaded', () => {
  setupModalForm({
    formSelector: '.join-us-form-row',
    statusSelector: '#join-us-status',
    modalSelector: '#joinSuccessModal',
    confettiSelector: '#joinConfetti',
    successMessage: "Success! We'll be in touch soon.",
  })
  ...
  setupModalForm({
    ...
    modalSelector: '#contactSuccessModal',
    confettiSelector: '#contactConfetti',
  ...
  setupModalForm({
    formSelector: '.modern-contact-form',
    statusSelector: '#connect-form-status',
    modalSelector: '#connectSuccessModal',
    confettiSelector: '#connectConfetti',
    successMessage: 'Thanks for connecting! We will get back to you shortly.',
  })
})
